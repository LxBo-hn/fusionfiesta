...
class OrganizerDashboardApi extends StatefulWidget {
  static const String routeName = '/organizer';
  const OrganizerDashboardApi({super.key});

  @override
  State<OrganizerDashboardApi> createState() => _OrganizerDashboardApiState();
}

class _OrganizerDashboardApiState extends State<OrganizerDashboardApi> {
                            ...
                            child: events.isEmpty
                                ...
                                : ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      return _buildEventCard(event, store);
                                    },
                                  ),
                          ),
                        ],
          ...
          child: Row(
            children: [
              ...
              PopupMenuButton<String>(
                onSelected: (value) => _handleEventAction(value, event, store),
                itemBuilder: (context) => [
                  const PopupMenuItem(
                    value: 'edit',
                    child: Row(
  ...
  Widget _buildMediaTab() {
    return Consumer<OrganizerStore>(
      builder: (context, store, child) {
        return ValueListenableBuilder<List<String>>(
          valueListenable: store.mediaAssets,
  ...
  void _handleEventAction(String action, EventModel event, OrganizerStore store) async {
    switch (action) {
      case 'edit':
        _showEditEventDialog(context, event, store);
        break;
      case 'qr':
        Navigator.pushNamed(context, RegistrationQRScreen.routeName);
        break;
      case 'delete':
        _showDeleteConfirmation(context, event, store);
        break;
    }
  }
  ...
  void _showEditEventDialog(BuildContext context, EventModel event, OrganizerStore store) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
  ...
  void _showDeleteConfirmation(BuildContext context, EventModel event, OrganizerStore store) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(context);
              final success = await store.deleteEvent(event.id);
              if (!success && mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(store.error.value ?? 'Failed to delete event'),
