...
class _EditEventScreenState extends State<EditEventScreen> {
  ...
  void _initializeFields() {
    ...
    _waitlistEnabled = widget.event.waitlistEnabled == 1;
    _selectedCategoryId = widget.event.categoryId;
    _selectedDepartmentId = widget.event.departmentId;
  ...
  Future<void> _updateEvent() async {
    ...
    try {
      ...
      final response = await ApiService.instance.put('/events/${widget.event.id}', eventData);
      ...
      if (response['success'] == true) {
        ...
        if (mounted) {
          _showErrorDialog(response['message'] ?? 'Có lỗi xảy ra khi cập nhật sự kiện');
        }
      }
    } catch (e) {
      if (mounted) {
  ...
  void _showSuccessDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Sự kiện đã được cập nhật thành công!'),
        actions: [
          TextButton(
            onPressed: () {
